  ...
  async (parcels: Parcel[], options: CommandOptions) => {
    ...
    for (const packageName of packageNames) {
      if (!allPackagesObj[packageName]) {
        throw new Error(`Package with provided name ${green.bold(packageName)} does not exist.`);
      }
    }
...
export const updateBundledNativeModulesFile = new Task<TaskArgs>(
  ...
  async (parcels: Parcel[]) => {
    ...
    for (const { pkg, state } of parcels) {
      ...
      if (!currentRange) {
        logger.log('  ', green.bold(pkg.packageName), gray('is not defined.'));
        continue;
      }
  ...
  async (parcels: Parcel[]) => {
    ...
    await Promise.all(
      Object.entries(workspaceInfo).map(async ([projectName, projectInfo]) => {
        ...
        batch.log('  ', green.bold(projectName));
    ...
    await Promise.all(
      parcels.map(async ({ pkg, changelog, state }) => {
        if (!(await changelog.fileExistsAsync())) {
          logger.log('  ', green.bold(pkg.packageName), gray(`- skipped, no changelog file.`));
          return;
        }
